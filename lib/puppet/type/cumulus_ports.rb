Puppet::Type.newtype(:cumulus_ports) do
  ...
  newproperty(:ensure) do
    ...
    def retrieve
      ...
      if prov and prov.respond_to?(:config_changed?)
        ...
        raise Puppet::DevError, errormsg
      end
      Puppet.debug "config sync result #{result}"
    ...
    newvalue :insync do
      prov = @resource.provider
      if prov and prov.respond_to?(:update_config)
        prov.update_config
      else
        errormsg = 'unable to find a provider for cumulus_ports ' +
